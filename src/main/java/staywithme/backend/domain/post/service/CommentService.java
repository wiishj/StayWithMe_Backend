...
public class CommentService {
    ...
    public CommentResponseDTO saveComment(CommentRequestDTO request, Member member) throws BadRequestException {
        ...
        if(request.getPostType().equals("Communication")){
            Communication post = communicationRepository.findById(request.getPostId()).orElseThrow();
            entity.setCommunication(post);
    ...
    public void deleteComment(Long commentId) {
        Comment entity = commentRepository.findById(commentId)
                .orElseThrow();
//        if(request.getPostType().equals("Communication")){
//            Communication post = entity.getCommunication();
//            post.removeComment(entity);
//        }else if(request.getPostType().equals("ClubDetail")){
//            ClubDetail post = entity.getClubDetail();
//            post.removeComment(entity);
//        }
//        member.removeComment(entity);
        commentRepository.delete(entity);
    }

    public List<CommentResponseDTO> getCommentByPostId(int type, Long postId) throws BadRequestException {
        if(type==0){
            Communication post = communicationRepository.findById(postId).orElseThrow();
            List<Comment> commentList = commentRepository.findByCommunication(post);
